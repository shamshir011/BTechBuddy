...
import com.example.btechbuddy.Adapter.DataStructureAdapter
...
class DataStructureActivity : AppCompatActivity() {
    private val binding: ActivityDataStructureBinding by lazy {
        ActivityDataStructureBinding.inflate(layoutInflater)
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?){
        ...
        supportActionBar!!.title = "Data Structure"
        retrieveDataStructureData()
    }
    private fun retrieveDataStructureData(){
        database = FirebaseDatabase.getInstance()
        quesAnsReference = database.reference.child("DataStructure")
            ...
            override fun onDataChange(snapshot: DataSnapshot) {
                for(quesAnsSnapshot in snapshot.children){
                    ...
                    quesAnsMaterial?.let {
                        ...
                        binding.dataStructureRecyclerView.visibility = View.VISIBLE
                    }
                }
                setAdapter()
            }
    ...
    private fun setAdapter(){
        if(quesAnsList.isNotEmpty()){
            ...
            binding.dataStructureRecyclerView.layoutManager = LinearLayoutManager(applicationContext)
            binding.dataStructureRecyclerView.adapter = adapter
