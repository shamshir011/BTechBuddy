...
import com.example.btechbuddy.Adapter.OperatingSystemAdapter
...
class OperatingSystemActivity : AppCompatActivity() {
    private val binding: ActivityOperatingSystemBinding by lazy {
        ActivityOperatingSystemBinding.inflate(layoutInflater)
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        retrieveOperatingSystemData()

    }
    private fun retrieveOperatingSystemData(){
        database = FirebaseDatabase.getInstance()
        quesAnsReference = database.reference.child("OperatingSystem")
            ...
            override fun onDataChange(snapshot: DataSnapshot) {
                for(quesAnsSnapshot in snapshot.children){
                    ...
                    quesAnsMaterial?.let {
                        ...
                        binding.operatingSystemRecyclerView.visibility = View.VISIBLE
                    }
                }
                setAdapter()
            }
    ...
    private fun setAdapter(){
        if(quesAnsList.isNotEmpty()){
            ...
            binding.operatingSystemRecyclerView.layoutManager = LinearLayoutManager(applicationContext)
            binding.operatingSystemRecyclerView.adapter = adapter
